...
entity uart_rx_dynamic_v2_tb is
--  Port ( );
end uart_rx_dynamic_v2_tb;

architecture Behavioral of uart_rx_dynamic_v2_tb is
...
SIGNAL rx_word_sig : STD_LOGIC_VECTOR(9 DOWNTO 0);
SIGNAL ready_sig : STD_LOGIC;                     
SIGNAL done_sig : STD_LOGIC;                     

procedure receive_uart_byte (
    signal rx : out std_logic;
    constant data  : std_logic_vector(8 downto 0); -- 8 max bits for data + 1 potential parity bit in MSb always
...
begin
    s_tick_sig <= '1'; -- whatever s_tick frequency here, as long as it's lower than clock frequency
    WAIT FOR 20 ns;
    s_tick_sig <= '0';
    WAIT FOR 20 ns;
end process;
...
begin 
    ...
    -- 640 ns = 40 ns (s_tick period = sub-frequency of clock signal, here clk_period*2) * 16 (over-sampling factor)
    WAIT FOR 4000 ns; -- just to manually space different frames we will send in this directed test-case
    ...
    -- stop bits shall be 1, but we will set the first one to 0
    rx_in_sig <= '0';
    wait for 640ns;
    rx_in_sig <= '1';
    wait for 640ns;
